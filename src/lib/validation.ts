...
export const ValidationRuleSchema = z.object({
  ...
  type: z.enum(['requirement_api', 'requirement_data', 'requirement_task', 'stack_dependency']),
...
export class ValidationEngine {
  ...
  private async loadRulesFromDatabase(): Promise<void> {
    try {
      ...
      console.warn('Failed to load validation rules from database, using defaults:', error)
    }
  }
    ...
    const defaultRules: ValidationRule[] = [
      {
        ...
        description: 'Each functional requirement should have corresponding API endpoints',
      ...
      
      {
        ...
        description: 'Each requirement should have corresponding development tasks',
        ...
        rule: `
          function validate(prdContent, tasksContent) {
            ...
            return {
              passed: unmatched.length <= requirements.length * 0.2,
              details: {
                total: requirements.length,
      ...
      
      {
        ...
        rule: `
          function validate(artifacts) {
            ...
            terms.forEach(term => {
              const mentions = Object.entries(artifacts).filter(([_name, content]) =>
  ...
  private extractDataEntities(content: string): DataEntity[] {
  ...
  private extractFields(content: string, entityName: string): string[] {
    ...
    for (const line of lines) {
      if (line.includes(entityName) && (line.includes('Table') || line.includes('Entity'))) {
  ...
  private extractTasks(content: string): Task[] {
    ...
    for (const line of lines) {
      if (line.match(/^-\s*\[|^\*\s*\d+\./)) {
        const task = line.replace(/^-\s*\[.*?\]\s*/, '').replace(/^\*\s*\d+\.\s*/, '')
  ...
  private estimateComplexity(task: string): number {
    const complexityKeywords: Record<number, string[]> = {
  ...
  private extractStack(content: string): string[] {
    const stack: string[] = []
    const techKeywords = ['Next.js', 'React', 'TypeScript', 'Tailwind', 'Neon', 'Drizzle', 'PostgreSQL']
  ...
  async validateArtifacts(projectId: string, phase: string, artifacts: Record<string, ArtifactContent>): Promise<ValidationReport> {
    const results: ValidationResult[] = []
    
    // Prepare artifact content map
    const artifactContentMap: Record<string, string> = {}
    Object.entries(artifacts).forEach(([_key, artifact]) => {
    ...
    for (const rule of this.rules) {
      ...
      try {
        const validationResult = await this.runValidationRule(rule, artifactContentMap)
        ...
        results.push({
          ...
          affectedArtifacts: this.getAffectedArtifacts(rule.type, artifactContentMap),
          affectedRequirements: this.getAffectedRequirements(rule.type, artifactContentMap)
        })
  ...
  private async runValidationRule(rule: ValidationRule, artifacts: Record<string, string>): Promise<{passed: boolean, details?: unknown}> {
    // Create validation context
    const context = {
      prdContent: artifacts['PRD.md'] || artifacts['prd.md'] || '',
      apiSpecContent: artifacts['api-spec.json'] || artifacts['api_spec.json'] || '',
      dataModelContent: artifacts['data-model.md'] || artifacts['data_model.md'] || '',
      tasksContent: artifacts['tasks.md'] || artifacts['tasks.md'] || '',
      stackProposal: artifacts['stack-proposal.md'] || artifacts['stack_proposal.md'] || '',
      dependenciesContent: artifacts['DEPENDENCIES.md'] || artifacts['dependencies.md'] || '',
      artifacts: artifacts
    }
    
    // Execute validation rule
    const validationFunction = new Function('context', `
      ${rule.rule}
  ...
  private getAffectedArtifacts(ruleType: string, artifacts: Record<string, string>): string[] {
    ...
    return artifactMapping[ruleType as keyof typeof artifactMapping]?.filter(artifact => 
      artifacts[artifact] && artifacts[artifact].length > 0
    ) || []
  }

  private getAffectedRequirements(ruleType: string, artifacts: Record<string, string>): string[] {
  ...
  async getValidationHistory(projectId: string, limit = 10) {
    try {
