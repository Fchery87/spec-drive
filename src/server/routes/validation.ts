...
import { db } from '@/db'
import { validationRules, projectArtifacts } from '@/db/schema'
import { AuthenticatedRequest } from '../middleware/auth'
import { validationEngine, ValidationRuleSchema } from '@/lib/validation'
...
const CreateRuleSchema = ValidationRuleSchema
...
function calculateTrend(reports: unknown[]): 'improving' | 'declining' | 'stable' {
  if (reports.length < 2) return 'stable'

  const recent = reports.slice(0, 2) as Array<{passedRules: number; totalRules: number}>
...
router.post('/rules', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to create validation rule' })
  }
})
...
router.patch('/rules/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to update validation rule' })
  }
})
...
router.delete('/rules/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to delete validation rule' })
  }
})
...
router.post('/run', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (artifactIds && artifactIds.length > 0) {
      artifactQuery = artifactQuery.where(eq(projectArtifacts.id, artifactIds[0]))
    } else {
...
router.get('/reports/:projectId', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch validation reports' })
  }
})

// GET /api/validation/reports/report/:reportId - Get specific validation report
router.get('/reports/report/:reportId', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch validation report' })
  }
})
...
router.get('/dashboard/:projectId', async (req: Request, res: Response) => {
  try {
    ...
    const recentReports = await validationEngine.getValidationHistory(projectId, 5)

    // Calculate dashboard metrics
    const metrics = {
      totalValidations: recentReports.length,
      passedValidations: recentReports.filter((r: any) => r.overallStatus === 'pass').length,
      failedValidations: recentReports.filter((r: any) => r.overallStatus === 'fail').length,
      warningValidations: recentReports.filter((r: any) => r.overallStatus === 'warning').length,
      lastValidation: recentReports.length > 0 ? recentReports[0].createdAt : null,
    ...
    const ruleStats = {
      ...
      byType: {
        requirement_api: rules.filter(r => r.type === 'requirement_api').length,
        requirement_data: rules.filter(r => r.type === 'requirement_data').length,
        requirement_task: rules.filter(r => r.type === 'requirement_task').length,
        stack_dependency: rules.filter(r => r.type === 'stack_dependency').length
      },
      bySeverity: {
    ...
    res.status(500).json({ success: false, error: 'Failed to fetch validation dashboard' })
  }
})
...
router.post('/auto-run', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (phaseArtifacts.length === 0) {
      return res
        .status(404)
        .json({ success: false, error: 'No artifacts found for auto-validation' })
    }
    ...
    for (const artifact of phaseArtifacts) {
      const hasErrors = report.validationResults.some(result => 
        !result.passed && result.affectedArtifacts?.includes(artifact.artifactName)
      )
      ...
      await db
        ...
        .set({
          ...
          validationErrors: hasErrors ? JSON.stringify(report.validationResults.filter(r => !r.passed)) : null
        })
    ...
    res.status(500).json({ success: false, error: 'Failed to run auto-validation' })
  }
})
